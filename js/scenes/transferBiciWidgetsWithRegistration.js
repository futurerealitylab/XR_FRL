...
// Self-contained: the card layout is embedded below (DEFAULT_CARDS), no save file.
//
...
export const init = async model => {
   ...
   const CARD_TINT = [.7, .7, 1.4];          // multiply tint on card faces (cools the warm lighting)
   ...
   if (isHeadset) {
      ...
      // Pane: 4 thin border squares, parked below the floor until calibration
      // delivers an anchor matrix (then placed each frame in the animate loop).
      pane = model.add();
